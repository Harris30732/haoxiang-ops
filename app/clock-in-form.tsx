...
import { clockInAction, needsSalesReport } from "@/lib/actions/clock";
...
export function ClockInForm({
  ...
  useEffect(() => {
    needsSalesReport(store.id)
      .then(setNeedSales)
      .catch(() => setNeedSales(true)); // 失敗時保守當作需要
  }, [store.id]);
  ...
  async function handleSubmit() {
    if (!canSubmit || !blob || !geo.coord) return;
    setSubmitting(true);
    setError(null);
    try {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col gap-4 p-4">
      <header>
        <div className="text-sm text-neutral-500">上班打卡</div>
