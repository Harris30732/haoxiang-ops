...
const dl = rd.driveLink || '';  // ★ 提案1: 打卡照片連結
const results = [];
if (ra === 'open_store') {
  // 每人工時紀錄（備註含開攤照片連結）
  results.push({ json: { sheetAction: 'attendance', '日期': rd.workDay, '員工姓名': rd.nickname, '店名': rd.store, '上班時間': rd.currentTime, '下班時間': '', '工時': '', '備註': dl ? '開攤 ' + dl : '開攤' } });
  // 每日店鋪紀錄
...
if (ra === 'clockin') {
  // 備註含上班打卡照片連結
  results.push({ json: { sheetAction: 'attendance', '日期': rd.workDay, '員工姓名': rd.nickname, '店名': rd.store, '上班時間': rd.currentTime, '下班時間': '', '工時': '', '備註': dl || '' } });
}
if (ra === 'clockout') {
...
// ======================================================================
// ★ 提案3: Error Trigger + Discord 告警
// 部署前請將 DISCORD_WEBHOOK_URL 換成真實 Webhook URL
// ======================================================================
const DISCORD_WEBHOOK_URL = 'YOUR_DISCORD_WEBHOOK_URL_HERE';
{
  const errTrigger = {
    id: mkId('ErrorTrigger'),
    name: 'Error Trigger',
  ...
  const errAlert = {
    ...
    parameters: {
      ...
      jsonBody: JSON.stringify({
        content: '⚠️ **StoreOps Bot 執行失敗**\n📍 節點：`{{ $json.execution.lastNodeExecuted }}`\n❌ 錯誤：{{ $json.error.message }}\n⏰ 時間：{{ $now.toFormat("yyyy-MM-dd HH:mm:ss") }}'
      })
    }
  };
  nodes.push(errAlert);
  connections['Error Trigger'] = { main: [[{ node: '錯誤告警(Discord)', type: 'main', index: 0 }]] };
}
...
const workflow = {
  ...
  connections,
  pinData: {},
  // ★ 提案4: maxConcurrency=1 防並發競態（打卡量低，影響可忽略）
  settings: { executionOrder: 'v1', maxConcurrency: 1 },
  staticData: null,
  meta: v16.meta
};

const outPath = path.join(ROOT, 'workflows', 'line-clock-in-bot-v18-dual-state.json');
