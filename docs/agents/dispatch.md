...
- 產出超過 30 行（報告、長 diff 說明、彙整表）→ 落檔後回傳路徑＋5 行內摘要。
  落檔位置的判定：日後要查的（研究、審查報告）→ `docs/agents/reports/{日期}-{主題}.md`；
  一次性中間產物 → 主對話在派工 prompt 裡**明給一個絕對路徑**
  （subagent 不知道你的 scratchpad 在哪，不寫它就會亂放）。
...
- **重試上限**：同一子任務、同一方法，總共最多嘗試 2 次（haiku 只有 1 次，見上）。
  第 2 次仍失敗，禁止再用同樣方法試第 3 次——必須換方法、升級模型、或問使用者。
  這條與上面的升級規則是同一件事的兩面：sonnet「連錯 2 次」即已達上限，下一步就是升 opus。
...
**fallback**：若派 `verifier` 時回報「Agent type not found」（自訂 agent 定義只在
session 啟動時載入；定義檔是 session 中途才建立、或載入失敗時就會這樣），改派
`general-purpose` + `model: sonnet`，並把 `.claude/agents/verifier.md` 的內文（frontmatter
以下全部）原封貼進派工 prompt 當作它的指示。驗收效力視同 verifier。
